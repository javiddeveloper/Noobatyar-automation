...
وضعیت: **انجام شده — ۲۰۲۶-۰۷-۲۹**، روی سرور تازه‌نصب‌شده (Ubuntu 24.04 LTS، ۱ هسته، ~۹۰۰ مگابایت رم، ۱.۵ گیگ swap). این سند از این پس مرجع بازسازی/دیپلوی مجدد است، نه یک کار در حال انجام.

نتیجه:
- `https://api.noobatyar.ir` و `https://app.noobatyar.ir` هر دو بالا و با گواهی معتبر (SAN شامل هر دو دامنه، انقضا ۲۰۲۶-۱۰-۲۷).
- فایروال فقط ۲۲/۸۰/۴۴۳ باز؛ Portainer (پورت ۹۰۰۰) از بیرون بسته شده — دسترسی فقط از طریق تانل SSH.
- `seed_plans` و مایگریشن‌ها اجرا شده، اولین سوپریوزر ساخته شده.
- `GUNICORN_WORKERS=2` ست شده (نه پیش‌فرض ۴) چون سرور فقط ۱ هسته دارد.

**دو باگ در اسکریپت‌های دیپلوی که این بار کشف و رفع شد** (روی سرورهای بعدی دیگر نباید رخ بدهد، چون رفع در همین کامیت‌هاست):
1. `backend/fresh_server_setup.sh` — تولید `SECRET_KEY`/`PG_PASSWORD` با فال‌بک `tr | head -c N` زیر `set -o pipefail` بود؛ وقتی `python3` روی سرور نصب نبود (تصویر Ubuntu مینیمال)، `SIGPIPE` ناشی از بستن زودهنگام پایپ توسط `head` به‌عنوان شکست واقعی تلقی می‌شد و کل اسکریپت درست بعد از کلون‌شدن ریپو می‌مرد، قبل از اجرای hardening یا TLS.
2. `backend/server_hardening.sh` — نصب `iptables-persistent` روی این Ubuntu 24.04 باعث شد apt خودش پکیج `ufw` را حذف کند (قوانین سطح kernel سر جایش می‌ماند، ولی باینری/سرویس `ufw` از بین می‌رفت)، و فراخوانی بعدی `ufw status verbose` کل اسکریپت را زیر `set -e` متوقف می‌کرد. حالا اگر این اتفاق بیفتد، اسکریپت خودش `ufw` را دوباره نصب می‌کند.
...
**۱. کال‌بک زیبال را تغییر نده — عمدی است.**
در `backend/accounting/views.py` خطوط ۱۶۲ و ۲۲۱ آدرس کال‌بک هاردکد شده روی
`https://noobatyar.ir/home/payment-result` و `.../payment-result-addon`. این صفحه
روی وردپرسِ هاست اشتراکی ساخته شده، نتیجه‌ی تراکنش را نشان می‌دهد و با دیپ‌لینک
`noobatyar://payment/result` کاربر را به اپ برمی‌گرداند. طراحی درست است، دست نزن.
(متغیر `SITE_URL` در settings تعریف شده ولی استفاده نمی‌شود — بی‌ضرر است.)

اما **یک چیز را بعد از مرحله ۷ تست کن**: آن صفحه‌ی وردپرسی اگر برای وریفای تراکنش
به بک‌اند درخواست می‌زند و آدرسش `http://93.127.223.93/...` باشد، مرورگر آن را
به‌عنوان mixed content بلاک می‌کند (صفحه https است، درخواست http). نشانه‌اش این است
که صفحه روی «در حال پردازش…» می‌ماند. بعد از این دیپلوی آدرس آن درخواست باید
`https://api.noobatyar.ir/...` شود. یک خرید واقعی کوچک بزن و کنسول مرورگر را ببین.
